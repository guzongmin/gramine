...
int get_topology_info(struct pal_topo_info* topo_info) {
    ...
    bool is_numa_nodes_info_available = false;
    size_t nodes_cnt = 0;
    /* Get the CPU node number.
     * Some systems do not have the file, for example, Windows Subsystem for Linux.
     * We set the node number to 1 if the numa nodes information is unavailable.
     * So, we ignor the -ENOENT error.*/
    ...
    is_numa_nodes_info_available = (ret == 0);
    ...
    if (is_numa_nodes_info_available == true) {
    ...
    if (is_numa_nodes_info_available == true) {
        ...
        /* Set the distance to the default value from Ubuntu 20.04. */
        distances[0] = 10;
