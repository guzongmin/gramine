...
ssize_t do_recvmsg(struct shim_handle* handle, struct iovec* iov, size_t iov_len, void* addr,
    ...
    if (flags & MSG_PEEK) {
        ...
        size_t size = 0;
        for (size_t i = 0; i < iov_len; i++) {
            /* This cannot overflow - we have already checked that all of this memory is present
             * so it must fit in `size_t`. */
            size += iov->iov_len;
        }
        if (sock->peek.data_size < size) {
            /* Try getting more data. */
            if (sock->peek.buf_size < size) {
                /* Reallocate the buffer. */
                void* peek_buf = malloc(size);
                if (!peek_buf) {
                    return -ENOMEM;
                }
                ...
                sock->peek.buf_size = size;
            }
            struct iovec* tmp_iov = {
                .iov_base = ,
                .iov_len = ,
            };
            ret = sock->ops->recv(handle, tmp_iov, 1, &tmp_iov.iov_len, NULL, NULL, /*is_blocking=*/false);
        }
#endif
    }
    ...
    size_t size = 0;
    ret = sock->ops->recv(handle, iov, iov_len, &size, addr, addrlen);
